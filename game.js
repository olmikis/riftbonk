(() => {
  ...
  function ensureThreeArenaRenderer(){
    ...
    try{
      ...
      function updateWeaponFx(time,rtx,visible){
        const projectileSource=visible?.projectiles||projectiles,beamSource=visible?.beams||beams,zoneSource=visible?.zones||zones,overloadAlpha=playerAttacksFaded?ATTACK_OVERLOAD_ALPHA:1;let projectileCount=0,beamCount=0,zoneCount=0,katanaCount=0;
        ...
        const zoneTypes={firetrail:0,gravity:1,frost:2,storm:3,seismic:4,pulse:4,meteor:5,mortar:5,mine:6,riftScar:7,blast:4};for(let i=0;i<zoneSource.length&&zoneCount<zoneFx.capacity;i++){const z=zoneSource[i];if(String(z.kind).startsWith('enemy')||z.kind==='katana')continue;const rgb=playerFxRgb(z.color||COLORS.cyan),type=zoneTypes[z.kind]??4,progress=type===4?clamp(1-z.life/Math.max(.01,z.max||1),0,1):clamp(z.life/Math.max(.01,z.max||1),0,1),alpha=overloadAlpha*(type===0?.42:type===1?.36:type===5?.55:.48),radius=Math.max(.15,z.radius||1);if(writeFx(zoneFx,zoneCount,[z.x,.075,z.z],[1,0],[radius*2,radius*2],[rgb[0],rgb[1],rgb[2],alpha],[type,progress,(z.seed||0)+i*.63,1]))zoneCount++;}
        ...
        finishFx(projectileFx,projectileCount,time,rtx);finishFx(beamFx,beamCount,time,rtx);finishFx(zoneFx,zoneCount,time,rtx);finishFx(katanaFx,katanaCount,time,rtx);canvas.dataset.shaderProjectiles=String(projectileCount);canvas.dataset.shaderBeams=String(beamCount);canvas.dataset.shaderZones=String(zoneCount);canvas.dataset.shaderKatana=String(katanaCount);
      }
      ...
      function updateArenaEffects(focus,theme,time,rtx,visible){
        ...
        const lightSources=[],zoneLights=visible?.zones||zones,projectileLights=visible?.projectiles||projectiles,enemyProjectileLights=visible?.enemyProjectiles||enemyProjectiles;for(const z of zoneLights){if((z.x-focus.x)**2+(z.z-focus.z)**2<24**2)lightSources.push({x:z.x,y:.7,z:z.z,color:z.color||COLORS.violet,hostile:String(z.kind).startsWith('enemy'),power:z.kind==='blast'||z.kind==='enemyBlast'?1.45:z.kind==='katana'?1.3:1});}for(const p of projectileLights){if((p.x-focus.x)**2+(p.z-focus.z)**2<20**2)lightSources.push({x:p.x,y:p.y||.7,z:p.z,color:p.color||COLORS.cyan,hostile:false,power:.72});}for(const p of enemyProjectileLights){if((p.x-focus.x)**2+(p.z-focus.z)**2<22**2)lightSources.push({x:p.x,y:p.y||.8,z:p.z,color:COLORS.red,hostile:true,power:.9});}lightSources.sort((a,b)=>(a.x-focus.x)**2+(a.z-focus.z)**2-((b.x-focus.x)**2+(b.z-focus.z)**2));
  ...
  const CODEX_WEAPONS = {
    ...
    katana:{summary:'Молниеносный дуговой удар рассекает всех врагов перед героем. Катана автоматически разворачивает взмах к захваченной или ближайшей цели.',stats:[['Урон','26 + 7 × (L − 1)'],['Дальность','4,40 + 0,35 × L'],['Полуугол дуги','0,72 + 0,025 × L рад'],['Перезарядка','(1,55 − 0,065 × L) / темп сек'],['Преграды','Не проходит сквозь препятствия']],impacts:['Урон героя','Темп атак','Размер атак','Урон по площади','Захват цели']}
  };
  ...
  const freshWeapons=()=>({blaster:1,aura:0,orbit:0,lightning:0,meteor:0,saw:0,frost:0,drone:0,gravity:0,firetrail:0,riftlance:0,boomerang:0,prism:0,mines:0,mortar:0,chrononeedles:0,seismic:0,nanoswarm:0,mirrordisc:0,resonance:0,katana:0});
  const freshCooldowns=()=>({blaster:.2,aura:1,lightning:2,meteor:3,saw:2,frost:3,drone:1,gravity:5,firetrail:.3,riftlance:1,boomerang:1,prism:.2,mines:1,mortar:2,chrononeedles:.4,seismic:1.5,nanoswarm:1,mirrordisc:.2,resonance:1,katana:.8});
  ...
  const AREA_DAMAGE_SOURCES=new Set(['aura','meteor','frost','firetrail','storm','voidPressure','corpseFire','killnova','critburst','riftScar','mines','mineLink','mortar','seismic','resonance','katana','pyroblast','guardianNova','mimicSpecial']);
  ...
  function fireKatana() {
    const l=weaponLevel('katana');if(!l)return;const evolved=hasEvolution('katana'),range=areaRadius((4.4+l*.35)*(evolved?1.18:1)),target=aimedEnemy(player.x,player.z,range+2.5);if(!target)return;const aim=Math.atan2(target.z-player.z,target.x-player.x),halfArc=(.72+l*.025)*(evolved?1.25:1),damage=(26+(l-1)*7)*(evolved?1.6:1),copies=1+echoCopies();player.dir=aim;
