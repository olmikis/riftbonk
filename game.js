(() => {
  ...
  const ENEMY_SPAWN_SCALING_RATE = .738;
  ...
  const CODEX_WEAPONS = {
    ...
    orbit:{summary:'Клинки постоянно вращаются вокруг героя и повторно задевают проходящих через орбиту врагов. Каждый дополнительный снаряд от «Двойной ставки» создаёт полноценный видимый клинок.',stats:[['Урон касания','7 + 3,2 × L'],['Клинки','2 + ⌊L / 2⌋ + бонус снарядов'],['Эволюция','Ещё +3 клинка'],['Радиус орбиты','2,15 + 0,12 × L'],['Интервал по цели','0,18 сек']],impacts:['Урон героя','Снаряды','Размер атак','Длительность эффектов']},
    ...
    katana:{summary:'Молниеносный дуговой удар рассекает всех врагов перед героем. Катана всегда бьёт строго в направлении взгляда персонажа и не использует автонаведение или захват цели.',stats:[['Урон','26 + 7 × (L − 1)'],['Дальность','4,40 + 0,35 × L'],['Полуугол дуги','0,72 + 0,025 × L рад'],['Перезарядка','(1,55 − 0,065 × L) / темп сек'],['Преграды','Не проходит сквозь препятствия']],impacts:['Урон героя','Темп атак','Размер атак','Урон по площади','Направление взгляда']},
    flamethrower:{summary:'Непрерывная расширяющаяся струя пламени всегда направлена по взгляду героя, наносит частые попадания и оставляет на врагах самостоятельное горение.',stats:[['Урон струи','3 + 1,05 × (L − 1)'],['Интервал струи','(0,15 − 0,005 × L) / темп сек'],['Дальность','5,80 + 0,32 × L'],['Полуугол конуса','0,30 + 0,018 × L рад'],['Горение','2 + 0,65 × (L − 1) каждые 0,36 сек'],['Длительность горения','1,25 + 0,07 × L сек'],['Белый фосфор','+20% струе и горению за ранг'],['Преграды','Пламя не проходит сквозь препятствия']],impacts:['Урон героя','Темп атак','Размер атак','Огненный урон','Белый фосфор','Направление взгляда']}
  };
  ...
  const upgrades = [
    ...
    {id:'burnPower',name:'Белый фосфор',icon:'△',type:'ХАРАКТЕРИСТИКА',rarity:'rare',max:6,desc:()=>'+20% урона Огненного следа, горящих луж, струи Огнемёта и оставленного ею горения.',apply:()=>state.stats.burnPower++},
    ...
    {id:'riftStabilizer',name:'Стабилизатор Разлома',icon:'⌛+',type:'ХАРАКТЕРИСТИКА',rarity:'rare',max:5,desc:()=>'+8% длительности зон. Мгновенные области оставляют остаточное поле для новых целей с 30% урона; создание зон на 5% реже за ранг.',apply:()=>state.stats.riftStabilizer++},
  ...
  const CODEX_ITEM_NOTES = {
    ...
    multishot:'Полный бонус получают обычные снаряды, пилы, бумеранги, отражения и Орбитальные клинки. Каждый ранг создаёт один дополнительный видимый клинок с полноценными попаданиями. Дополнительные метеоры, миномётные заряды и лучи наносят 50% урона; дополнительные начальные молнии — 55%. Мины, саммоны и постоянные зоны не дублируются.',
    ...
    burnPower:'Каждый ранг добавляет +20% от базового огненного урона отдельно струе Огнемёта и её последующему горению. Этот же бонус продолжает усиливать Огненный след и огонь от трупов.',
    riftStabilizer:'Нулевая аура, Метеоритный дождь, Крио-импульс, Ионный миномёт, Сейсмическое ядро и Резонансный колокол оставляют поле, которое один раз поражает только новых вошедших врагов. Крио-импульс также получает увеличенную длительность замедления.',
