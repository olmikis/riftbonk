(() => {
  ...
  const ENEMY_DAMAGE_SCALING_RATE = .35;
  const ENEMY_SPAWN_SCALING_RATE = .6273;
  ...
  const JUMP_VELOCITY = 7.2, JUMP_GRAVITY = 21.5, JUMP_BUFFER = .14, BHOP_GAIN = .06, BHOP_MAX_SPEED = 1.3, BHOP_GRACE = .16;
  const ADAPTIVE_HEALTH_MAX = 1.40;
  const TELEMETRY_ENABLED = false;
  const BALANCE_REPORT_VERSION = 2, BALANCE_SAMPLE_INTERVAL = 5;
  const RENDER_SCALE_DEFAULT = .8, RENDER_SCALE_MIN = .5, RENDER_SCALE_MAX = 1, ATTACK_OPACITY_DEFAULT = 1, ATTACK_OPACITY_MIN = .1;
  let renderScale=RENDER_SCALE_DEFAULT,attackOpacity=ATTACK_OPACITY_DEFAULT;
  ...
  try{const storedAttackOpacity=localStorage.getItem('riftAttackOpacity'),savedAttackOpacity=Number(storedAttackOpacity);if(storedAttackOpacity!==null&&Number.isFinite(savedAttackOpacity))attackOpacity=clamp(savedAttackOpacity,ATTACK_OPACITY_MIN,1);}catch(_error){}
  const RUN_PACES = {
  ...
  function setAttackOpacity(value,persist=true){
    const numeric=Number(value),next=Number.isFinite(numeric)?numeric:ATTACK_OPACITY_DEFAULT;attackOpacity=Math.round(clamp(next,ATTACK_OPACITY_MIN,1)*20)/20;
    const percent=Math.round(attackOpacity*100),input=$('#attackOpacityInput'),label=$('#attackOpacityValue');if(input&&Number(input.value)!==percent)input.value=String(percent);if(label)label.textContent=`${percent}%`;
    canvas.dataset.attackOpacitySetting=String(attackOpacity);if(persist)try{localStorage.setItem('riftAttackOpacity',String(attackOpacity));}catch(_error){}
  }
  ...
  function ensureThreeArenaRenderer(){
    ...
    try{
      ...
      function updateMonsterModels(visuals,time){
        ...
        for(let i=0;i<monsterPool.length;i++){const entry=monsterPool[i],visual=i<count?source[i]:null;entry.root.visible=Boolean(visual);if(!visual)continue;const scale=Math.max(.2,Number(visual.size)||1);entry.root.position.set(Number(visual.x)||0,(Number(visual.y)||0)-.83*scale,Number(visual.z)||0);entry.root.rotation.set(0,Math.PI*.5-(Number(visual.dir)||0),0);entry.root.scale.setScalar(scale);if(visual.animate){animated++;if(tick&&entry.mixer)entry.mixer.update(animationDelta);}else frozen++;}
      ...
      const katanaFx=createWeaponFxLayer(768,`uniform float uTime;uniform float uQuality;varying vec2 vUv;varying vec4 vColor;varying vec4 vData;${fxNoise}void main(){vec2 p=vUv*2.0-1.0;float r=length(p);if(r>1.0)discard;float a=atan(p.y,p.x),progress=clamp(vData.x,0.0,1.0),evolved=vData.y,seed=vData.z,aperture=.92+evolved*.16,wedge=1.0-smoothstep(aperture,aperture+.11,abs(a)),front=smoothstep(-.16,.12,p.x),sweep=.22+progress*.76;float blade=smoothstep(.06,.0,abs(r-sweep))*wedge*front,trail=smoothstep(.18,.0,abs(r-(sweep-.13)))*wedge*front*(1.0-progress*.45),second=evolved*smoothstep(.045,.0,abs(r-(sweep-.19)))*wedge*front;float grain=noise(vec2(a*10.0+seed,r*31.0-uTime*7.0)),sparks=smoothstep(.83,1.0,grain)*smoothstep(.16,.92,r)*wedge*front*(1.0-progress)*mix(.42,1.0,uQuality),tip=smoothstep(.16,.0,length(vec2(r-sweep,(abs(a)-aperture)*.7)));float fade=1.0-smoothstep(.76,1.0,progress),alpha=(blade*1.18+trail*.34+second*.82+sparks*.34+tip*.45)*vColor.a*fade;if(alpha<.012)discard;vec3 spectral=mix(vColor.rgb,vec3(.94,1.0,1.0),clamp(blade+tip,0.0,1.0));spectral+=second*vec3(.48,.18,1.0)+sparks*vec3(.28,.82,1.0);gl_FragColor=vec4(spectral*(.74+blade*2.15+second*1.25),clamp(alpha,0.0,1.0));}`,'disc');
      const flamethrowerFx=createWeaponFxLayer(768,`uniform float uTime;uniform float uQuality;varying vec2 vUv;varying vec4 vColor;varying vec4 vData;${fxNoise}void main(){float x=clamp(vUv.x,0.0,1.0),y=vUv.y*2.0-1.0,life=clamp(vData.x,0.0,1.0),evolved=vData.y,seed=vData.z;float flow=noise(vec2(x*8.0-uTime*7.5+seed,y*4.5+seed)),detail=noise(vec2(x*19.0-uTime*13.0-seed,y*11.0+uTime*1.8));float sway=(flow-.5)*(.10+x*.22),width=mix(.12,.88,pow(x,.72))*(.86+flow*.22+evolved*.10),distance=abs(y+sway),body=1.0-smoothstep(width*.58,width,distance);float nozzle=smoothstep(0.0,.055,x),ragged=1.0-smoothstep(.70,1.03,x+(detail-.5)*.30),tongues=smoothstep(.46,.94,detail+(.76-distance/max(.05,width))*.28)*smoothstep(.14,.96,x);float envelope=body*nozzle*ragged,core=(1.0-smoothstep(width*.08,width*.38,distance))*smoothstep(0.0,.12,x)*(1.0-smoothstep(.58,1.0,x)),embers=smoothstep(.88,.985,detail)*body*smoothstep(.18,.92,x)*mix(.35,1.0,uQuality);float flicker=.78+.22*sin(uTime*31.0+seed*9.0+x*18.0),fade=.65+.35*life,alpha=(envelope*(.55+tongues*.46)+embers*.55)*vColor.a*fade*flicker;if(alpha<.012)discard;vec3 outer=vec3(1.0,.26,.025),middle=vec3(1.0,.68,.08),hot=evolved>0.5?vec3(.88,.98,1.0):vec3(1.0,.96,.58),color=mix(outer,middle,clamp((1.0-distance/max(.02,width))*.82,0.0,1.0));color=mix(color,hot,clamp(core+embers*.28,0.0,1.0));color+=embers*vec3(.55,.42,.10);gl_FragColor=vec4(color*(.80+core*2.15+tongues*.45),clamp(alpha,0.0,1.0));}`);
      ...
      function updateWeaponFx(time,rtx,visible){
        const projectileSource=visible?.projectiles||projectiles,beamSource=visible?.beams||beams,zoneSource=visible?.zones||zones,overloadAlpha=(playerAttacksFaded?ATTACK_OVERLOAD_ALPHA:1)*attackOpacity;let projectileCount=0,beamCount=0,zoneCount=0,katanaCount=0,flamethrowerCount=0;
        ...
        const zoneTypes={firetrail:0,gravity:1,frost:2,storm:3,seismic:4,pulse:4,meteor:5,mortar:5,mine:6,riftScar:7,blast:4};for(let i=0;i<zoneSource.length&&zoneCount<zoneFx.capacity;i++){const z=zoneSource[i];if(String(z.kind).startsWith('enemy')||z.kind==='katana'||z.kind==='flamethrower')continue;const rgb=playerFxRgb(z.color||COLORS.cyan),type=zoneTypes[z.kind]??4,progress=type===4?clamp(1-z.life/Math.max(.01,z.max||1),0,1):clamp(z.life/Math.max(.01,z.max||1),0,1),alpha=overloadAlpha*(type===0?.42:type===1?.36:type===5?.55:.48),radius=Math.max(.15,z.radius||1);if(writeFx(zoneFx,zoneCount,[z.x,.075,z.z],[1,0],[radius*2,radius*2],[rgb[0],rgb[1],rgb[2],alpha],[type,progress,(z.seed||0)+i*.63,1]))zoneCount++;}
        for(let i=0;i<zoneSource.length&&katanaCount<katanaFx.capacity;i++){const z=zoneSource[i];if(z.kind!=='katana')continue;const rgb=playerFxRgb(z.color||COLORS.cyan),dir=Number(z.dir)||0,radius=Math.max(.3,z.radius||1),progress=clamp(1-z.life/Math.max(.01,z.max||1),0,1);if(writeFx(katanaFx,katanaCount,[z.x,.13,z.z],[Math.cos(dir),Math.sin(dir)],[radius*2,radius*2],[rgb[0],rgb[1],rgb[2],overloadAlpha*.94],[progress,z.evolved?1:0,(z.seed||0)+i*.47,1]))katanaCount++;}
        for(let i=0;i<zoneSource.length&&flamethrowerCount<flamethrowerFx.capacity;i++){const z=zoneSource[i];if(z.kind!=='flamethrower')continue;const dir=Number(z.dir)||0,length=Math.max(.2,z.length||z.radius||1),width=Math.max(.3,z.width||1),progress=clamp(z.life/Math.max(.01,z.max||1),0,1);if(writeFx(flamethrowerFx,flamethrowerCount,[z.x,.16,z.z],[Math.cos(dir),Math.sin(dir)],[length,width],[1,.48,.055,overloadAlpha*.82],[progress,z.evolved?1:0,(z.seed||0)+i*.39,1]))flamethrowerCount++;}
        finishFx(projectileFx,projectileCount,time,rtx);finishFx(beamFx,beamCount,time,rtx);finishFx(zoneFx,zoneCount,time,rtx);finishFx(katanaFx,katanaCount,time,rtx);finishFx(flamethrowerFx,flamethrowerCount,time,rtx);canvas.dataset.shaderProjectiles=String(projectileCount);canvas.dataset.shaderBeams=String(beamCount);canvas.dataset.shaderZones=String(zoneCount);canvas.dataset.shaderKatana=String(katanaCount);canvas.dataset.shaderFlamethrower=String(flamethrowerCount);
      }
      ...
      function updateArenaEffects(focus,theme,time,rtx,visible){
        ...
        const lightSources=[],zoneLights=visible?.zones||zones,projectileLights=visible?.projectiles||projectiles,enemyProjectileLights=visible?.enemyProjectiles||enemyProjectiles;for(const z of zoneLights){if((z.x-focus.x)**2+(z.z-focus.z)**2<24**2)lightSources.push({x:z.x,y:z.kind==='flamethrower'?.42:.7,z:z.z,color:z.color||COLORS.violet,hostile:String(z.kind).startsWith('enemy'),power:z.kind==='blast'||z.kind==='enemyBlast'?1.45:z.kind==='flamethrower'?1.65:z.kind==='katana'?1.3:1});}for(const p of projectileLights){if((p.x-focus.x)**2+(p.z-focus.z)**2<20**2)lightSources.push({x:p.x,y:p.y||.7,z:p.z,color:p.color||COLORS.cyan,hostile:false,power:.72});}for(const p of enemyProjectileLights){if((p.x-focus.x)**2+(p.z-focus.z)**2<22**2)lightSources.push({x:p.x,y:p.y||.8,z:p.z,color:COLORS.red,hostile:true,power:.9});}lightSources.sort((a,b)=>(a.x-focus.x)**2+(a.z-focus.z)**2-((b.x-focus.x)**2+(b.z-focus.z)**2));
  ...
  const WEAPON_INFO = {
    ...
    katana:{name:'Катана',icon:'刃',color:'violet'},
    flamethrower:{name:'Огнемёт',icon:'♨',color:'amber'}
  };
  const CODEX_WEAPONS = {
    ...
    aura:{summary:'Периодический взрыв вокруг героя. Не требует цели и особенно силён против окружившей толпы.',stats:[['Урон','9 + 6 × L'],['Перезарядка','(1,70 − 0,07 × L) / темп сек'],['Радиус','3,10 + 0,48 × L'],['Тип','Зона вокруг героя']],impacts:['Урон героя','Темп атак','Размер атак','Стабилизатор Разлома']},
    orbit:{summary:'Вращающиеся клинки многократно задевают врагов рядом с героем.',stats:[['Урон касания','7 + 3,2 × L'],['Клинки','2 + ⌊L / 2⌋ + бонус снарядов'],['Эволюция','Ещё +3 клинка'],['Радиус орбиты','2,15 + 0,12 × L'],['Интервал по цели','0,18 сек']],impacts:['Урон героя','Снаряды','Размер атак','Длительность эффектов']},
    lightning:{summary:'Молния перескакивает между ближайшими целями.',stats:[['Урон','18 + 8 × L'],['Начальные дуги','1 + бонус снарядов'],['Урон дополнительных дуг','55%'],['Цели каждой цепи','2 + L + 2 × бонус цепи'],['Дальность прыжка','5,50 + 0,25 × L'],['Перезарядка','(2,50 − 0,11 × L) / темп сек']],impacts:['Урон героя','Темп атак','Снаряды','Цепь молний']},
    meteor:{summary:'Обрушивает метеоры на самые плотные скопления врагов.',stats:[['Урон','28 + 12 × L'],['Основные метеоры','min(1 + ⌊L / 3⌋, 5)'],['Дополнительные метеоры','бонус снарядов · 50% урона'],['Радиус','2,20 + 0,28 × L'],['Перезарядка','(4,20 − 0,18 × L) / темп сек']],impacts:['Урон героя','Темп атак','Снаряды','Размер атак','Урон по площади','Стабилизатор Разлома']},
    ...
    frost:{summary:'Круговой импульс наносит урон и надолго замедляет всех задетых противников.',stats:[['Урон','12 + 6 × L'],['Радиус','4,00 + 0,48 × L'],['Замедление','min(42% + 3,5% × L, 72%)'],['Длительность','2,00 + 0,35 × L сек'],['Перезарядка','(3,40 − 0,12 × L) / темп сек']],impacts:['Урон героя','Темп атак','Размер атак','Длительность','Урон по замедленным','Стабилизатор Разлома']},
    ...
    gravity:{summary:'Стягивает врагов в одну точку; урон наносит только Пустотник.',stats:[['Урон','0 · Пустотник: 4 + 1,5 × L каждые 0,45 сек'],['Радиус','3,20 + 0,35 × L'],['Длительность','2,50 + 0,30 × L сек'],['Сила притяжения','3,50 + 0,70 × L'],['Перезарядка','2 × (6,00 − 0,22 × L) / темп сек'],['Боссы','15% обычного притяжения']],impacts:['Темп атак','Размер атак','Длительность','Контроль толпы','Пассив Пустотника']},
    firetrail:{summary:'Во время движения оставляет за героем горящие зоны.',stats:[['Урон тика','5 + 2,8 × L'],['Интервал тика','0,30 сек'],['Радиус','1,05 + 0,10 × L'],['Длительность','2,20 + 0,28 × L сек'],['Шаг создания','0,48 − 0,025 × L сек']],impacts:['Урон героя','Огненный урон','Темп зон','Размер атак','Длительность']},
    riftlance:{summary:'Длинный луч прошивает линию врагов перед героем.',stats:[['Урон','32 + 7 × (L − 1)'],['Лучей','1 + бонус снарядов'],['Урон дополнительных лучей','50%'],['Длина','30 + 0,70 × L'],['Ширина','0,34 + 0,035 × L'],['Лимит целей','8 + (L − 1) + пробитие'],['Перезарядка','(1,60 − 0,045 × L) / темп сек']],impacts:['Урон героя','Темп атак','Снаряды','Пробитие','Калибр']},
    ...
    prism:{summary:'Непрерывный луч усиливается, пока бьёт одну цель.',stats:[['Урон тика','2,65 + 0,70 × L'],['Дополнительные лучи','бонус снарядов · 50% урона'],['Интервал тика','0,18 / темп сек'],['Максимальный разгон','×1,50'],['Дальность','27'],['Разгон за тик','5,5% + 0,8% × L']],impacts:['Урон героя','Темп атак','Снаряды','Метка преследования']},
    mines:{summary:'Оставляет рядом с героем мины, срабатывающие при приближении врагов.',stats:[['Урон','35 + 7 × (L − 1)'],['Мин за установку','1'],['Радиус взрыва','2,20 + 0,08 × L'],['Радиус детонации','1,15 + 0,04 × L'],['Максимум мин','5 + ⌊L / 2⌋'],['Длительность','12 + 0,45 × L сек'],['Перезарядка','(1,30 − 0,035 × L) / темп сек']],impacts:['Урон героя','Темп атак','Размер атак','Длительность','Урон по площади']},
    mortar:{summary:'Обстреливает наиболее плотную группу врагов.',stats:[['Урон','42 + 9 × (L − 1)'],['Дополнительные попадания','бонус снарядов · 50% урона'],['Радиус','2,70 + 0,13 × L'],['Перезарядка','(2,40 − 0,08 × L) / темп сек'],['Бонус от плотности','до −35% перезарядки']],impacts:['Урон героя','Темп атак','Снаряды','Размер атак','Урон по площади','Стабилизатор Разлома']},
    ...
    mirrordisc:{summary:'Диск ранит врагов рядом с героем и отражает часть вражеских снарядов.',stats:[['Урон касания','8 + 4 × L'],['Интервал по цели','0,42 сек'],['Отражённые снаряды','1 + бонус снарядов'],['Урон отражения','50% урона врага × (1 + 0,08 × L)'],['Пробитие отражения','2 + ⌊L / 3⌋'],['Интервал отражения','max(0,30; 1,25 − 0,10 × L) сек']],impacts:['Урон героя','Снаряды','Калибр','Защита от снарядов','Отражение']},
    resonance:{summary:'Каждый следующий импульс усиливается. Получение урона сбрасывает половину накопленного резонанса.',stats:[['Базовый урон','12 + 4 × (L − 1)'],['Бонус за заряд','+15%'],['Максимум зарядов','10'],['Радиус','4,40 + 0,32 × L'],['Перезарядка','(1,50 − 0,04 × L) / темп сек']],impacts:['Урон героя','Темп атак','Размер атак','Серия без получения урона','Стабилизатор Разлома']},
    katana:{summary:'Дуговой удар поражает всех врагов перед героем.',stats:[['Урон','26 + 7 × (L − 1)'],['Дальность','4,40 + 0,35 × L'],['Полуугол дуги','0,72 + 0,025 × L рад'],['Перезарядка','(1,55 − 0,065 × L) / темп сек'],['Преграды','Не проходит сквозь препятствия']],impacts:['Урон героя','Темп атак','Размер атак','Урон по площади','Направление взгляда']},
    flamethrower:{summary:'Непрерывная струя поражает сектор перед героем и поджигает врагов.',stats:[['Урон струи','3 + 1,05 × (L − 1)'],['Интервал струи','(0,15 − 0,005 × L) / темп сек'],['Дальность','5,80 + 0,32 × L'],['Полуугол конуса','0,30 + 0,018 × L рад'],['Горение','2 + 0,65 × (L − 1) каждые 0,36 сек'],['Длительность горения','1,25 + 0,07 × L сек'],['Белый фосфор','+20% струе и горению за ранг'],['Преграды','Пламя не проходит сквозь препятствия']],impacts:['Урон героя','Темп атак','Размер атак','Огненный урон','Белый фосфор','Направление взгляда']}
  };
  const DAMAGE_SOURCE_NAMES={blaster:'Осколочный залп',aura:'Нулевая аура',orbit:'Орбитальные клинки',lightning:'Цепь молний',storm:'Вечная гроза',meteor:'Метеоритный дождь',saw:'Пила разрыва',frost:'Крио-импульс',drone:'Дроны-охотники',gravity:'Гравитационный колодец',voidPressure:'Давление пустоты',firetrail:'Огненный след',flamethrower:'Огнемёт',flamethrowerBurn:'Огнемёт · горение',riftlance:'Копьё Разлома',riftScar:'Прокол Реальности',boomerang:'Призрачный бумеранг',prism:'Кристальный призматор',mines:'Минный фабрикатор',mineLink:'Нулевая территория',mortar:'Ионный миномёт',chrononeedles:'Хроно-иглы',seismic:'Сейсмическое ядро',nanoswarm:'Нанорой',mirrorDisc:'Зеркальный диск',reflected:'Отражённый снаряд',resonance:'Резонансный колокол',katana:'Катана',engineerTurret:'Турели Инженера',necrominion:'Мертвецы Некроманта',pyroblast:'Взрыв Пироманта',guardianNova:'Волна Стража',mimic:'Эхо Мимика',mimicSpecial:'Украденная атака',discharge:'Разрядный конденсатор',shardburst:'Осколочный накопитель',unstableDuplicate:'Нестабильный дубликатор',corpseFire:'Пепел к пеплу',killnova:'Реактор бойни',critburst:'Критическая масса',thorns:'Ответный BONK',normal:'Прочий урон'};
  const DAMAGE_SOURCE_WEAPON={blaster:'blaster',aura:'aura',orbit:'orbit',lightning:'lightning',storm:'lightning',meteor:'meteor',saw:'saw',frost:'frost',drone:'drone',gravity:'gravity',voidPressure:'gravity',firetrail:'firetrail',flamethrower:'flamethrower',flamethrowerBurn:'flamethrower',riftlance:'riftlance',riftScar:'riftlance',boomerang:'boomerang',prism:'prism',mines:'mines',mineLink:'mines',mortar:'mortar',chrononeedles:'chrononeedles',seismic:'seismic',nanoswarm:'nanoswarm',mirrorDisc:'mirrordisc',reflected:'mirrordisc',resonance:'resonance',katana:'katana'};
  ...
  let selectedGraphics='normal',selectedRunPace='standard',selectedChallenge='classic',selectedSeed='',selectedMap='normal',combatNumbersEnabled=true;
  ...
  try{combatNumbersEnabled=localStorage.getItem('riftCombatNumbers')!=='off';}catch(_error){}
  ...
  let enemies=[],projectiles=[],enemyProjectiles=[],gems=[],particles=[],beams=[],zones=[],consumables=[],worldTotems=[],heroUnits=[],obstacles=[],obstacleGrid=new Map(),generatedObstacleCells=new Set(),obstacleGenerationX=Infinity,obstacleGenerationZ=Infinity,platforms=[],platformGroups=[],platformGrid=new Map(),platformGroupById=new Map(),generatedPlatformCells=new Set(),platformGenerationX=Infinity,platformGenerationZ=Infinity,mazeFlowField=new Map(),mazeFlowKey='',activeStandards=[],playerAttacksFaded=false,aoeVisualCulling=false,visibleAoeTotal=0,visibleAoeCulled=0,nextZoneVisualId=1,lastViewProjection=null,lastViewFirstPerson=false,combatTextBuckets=new Map(),combatTexts=[];
  ...
  let keys={},jumpQueued=false,touchMove={x:0,z:0},choiceKind='level',currentChoices=[],choiceActionMode='';
  ...
  function spawnPlayerProjectile(data){const entity=acquirePooled('projectile'),hit=entity.hit instanceof Set?entity.hit:new Set(),owner=actorById(data.owner),ownerBody=owner?.entity,nearOwner=ownerBody&&(Number(data.x)-ownerBody.x)**2+(Number(data.z)-ownerBody.z)**2<3.1**2;Object.assign(entity,data);if(nearOwner&&Number.isFinite(Number(entity.y)))entity.y+=Number(ownerBody.y)||0;hit.clear();entity.hit=hit;projectiles.push(entity);return entity;}
  ...
  function collectVisibleFrame(matrix,menu=false){
    ...
    for(const enemy of enemies)if(!enemy.dead&&clipVisible(matrix,enemy.x,(Number(enemy.y)||0)+Math.max(.7,enemy.size),enemy.z,enemy.size*2.5))visibleFrame.enemies.push(enemy);
  ...
  const freshWeapons=()=>({blaster:1,aura:0,orbit:0,lightning:0,meteor:0,saw:0,frost:0,drone:0,gravity:0,firetrail:0,riftlance:0,boomerang:0,prism:0,mines:0,mortar:0,chrononeedles:0,seismic:0,nanoswarm:0,mirrordisc:0,resonance:0,katana:0,flamethrower:0});
  const freshCooldowns=()=>({blaster:.2,aura:1,lightning:2,meteor:3,saw:2,frost:3,drone:1,gravity:5,firetrail:.3,riftlance:1,boomerang:1,prism:.2,mines:1,mortar:2,chrononeedles:.4,seismic:1.5,nanoswarm:1,mirrordisc:.2,resonance:1,katana:.8,flamethrower:.2});
  ...
  const PLATFORM_CELL_SIZE=30,PLATFORM_GRID_SIZE=12,PLATFORM_MAX_HALF=6,PLATFORM_STEP_HEIGHT=.76,PLATFORM_BODY_HEIGHT=2.15;
  ...
  const platformGridKey=(x,z)=>`${x}:${z}`;
  function addPlatformSurface(surface){
    platforms.push(surface);const minX=Math.floor((surface.x-surface.hx)/PLATFORM_GRID_SIZE),maxX=Math.floor((surface.x+surface.hx)/PLATFORM_GRID_SIZE),minZ=Math.floor((surface.z-surface.hz)/PLATFORM_GRID_SIZE),maxZ=Math.floor((surface.z+surface.hz)/PLATFORM_GRID_SIZE);for(let gx=minX;gx<=maxX;gx++)for(let gz=minZ;gz<=maxZ;gz++){const key=platformGridKey(gx,gz),bucket=platformGrid.get(key);if(bucket)bucket.push(surface);else platformGrid.set(key,[surface]);}
  }
  function platformSurfacesNear(x,z,radius=0){
    if(!platforms.length)return[];const reach=radius+PLATFORM_MAX_HALF,minX=Math.floor((x-reach)/PLATFORM_GRID_SIZE),maxX=Math.floor((x+reach)/PLATFORM_GRID_SIZE),minZ=Math.floor((z-reach)/PLATFORM_GRID_SIZE),maxZ=Math.floor((z+reach)/PLATFORM_GRID_SIZE),found=[],seen=new Set();for(let gx=minX;gx<=maxX;gx++)for(let gz=minZ;gz<=maxZ;gz++)for(const surface of platformGrid.get(platformGridKey(gx,gz))||[])if(!seen.has(surface)){seen.add(surface);found.push(surface);}return found;
  }
  function platformSupportAt(x,z,maxHeight=Infinity){
    let height=0,surface=null;for(const candidate of platformSurfacesNear(x,z,.05)){if(Math.abs(x-candidate.x)>candidate.hx+.03||Math.abs(z-candidate.z)>candidate.hz+.03||candidate.y>maxHeight+.04||candidate.y<=height)continue;height=candidate.y;surface=candidate;}return{height,surface};
  }
  function platformBodyHeight(entity){return Math.max(1.15,Number(entity?.bodyHeight)||Math.max(PLATFORM_BODY_HEIGHT,(Number(entity?.size)||1)*1.42));}
  function platformVolumeBlocks(entity,x,z,radius=0){
    const feet=Math.max(0,Number(entity?.y)||0),head=feet+platformBodyHeight(entity);for(const surface of platformSurfacesNear(x,z,radius)){if(Math.abs(x-surface.x)>=surface.hx+radius||Math.abs(z-surface.z)>=surface.hz+radius||surface.y<=feet+PLATFORM_STEP_HEIGHT+.04)continue;const bottom=surface.y-surface.thickness;if(bottom<head-.08&&surface.y>feet+.05)return true;}return false;
  }
  function platformFootprintBlocked(surfaces){return surfaces.some(surface=>nearbyObstacles(surface.x,surface.z,Math.max(surface.hx,surface.hz)).some(obstacle=>Math.abs(surface.x-obstacle.x)<surface.hx+obstacle.hx+.45&&Math.abs(surface.z-obstacle.z)<surface.hz+obstacle.hz+.45));}
  function generatePlatformCell(gx,gz){
    const cell=`${gx}:${gz}`;if(generatedPlatformCells.has(cell))return;generatedPlatformCells.add(cell);const holder={value:seedHash(`${state.seedCode}:platform-map:${cell}`)},next=()=>seededValue(holder);if(next()>.68)return;
    const x=gx*PLATFORM_CELL_SIZE+(next()-.5)*8,z=gz*PLATFORM_CELL_SIZE+(next()-.5)*8;if(x*x+z*z<12**2)return;const height=3.65+next()*.75,hx=3.55+next()*.85,hz=2.9+next()*.75,side=Math.floor(next()*4),outX=side===0?1:side===1?-1:0,outZ=side===2?1:side===3?-1:0,alongHalf=outX?hx:hz,stepCount=7,stepDepth=1.25,stepHalfWidth=1.25,id=`platform:${cell}`,surfaces=[{groupId:id,kind:'deck',x,z,hx,hz,y:height,thickness:.46,variant:Math.floor(next()*3)}];
    for(let index=1;index<=stepCount;index++){const top=height*index/stepCount,distance=alongHalf+(stepCount-index+.5)*stepDepth,sx=x+outX*distance,sz=z+outZ*distance;surfaces.push({groupId:id,kind:'step',x:sx,z:sz,hx:outX?stepDepth*.5:stepHalfWidth,hz:outZ?stepDepth*.5:stepHalfWidth,y:top,thickness:top,variant:Math.floor(next()*3)});}
    if(platformFootprintBlocked(surfaces))return;const low=surfaces[1],group={id,x,z,hx,hz,height,side,outX,outZ,surfaces,stairBase:{x:low.x+outX*stepDepth*.7,z:low.z+outZ*stepDepth*.7},stairTop:{x:x+outX*Math.max(.4,alongHalf-.8),z:z+outZ*Math.max(.4,alongHalf-.8)}};platformGroups.push(group);platformGroupById.set(id,group);for(const surface of surfaces)addPlatformSurface(surface);
  }
  function ensurePlatforms(x,z,force=false){
    const centerX=Math.floor(x/PLATFORM_CELL_SIZE),centerZ=Math.floor(z/PLATFORM_CELL_SIZE);if(!force&&Math.abs(centerX-platformGenerationX)<2&&Math.abs(centerZ-platformGenerationZ)<2)return;for(let gx=centerX-4;gx<=centerX+4;gx++)for(let gz=centerZ-4;gz<=centerZ+4;gz++)generatePlatformCell(gx,gz);platformGenerationX=centerX;platformGenerationZ=centerZ;canvas.dataset.platforms=String(platformGroups.length);
  }
  function generatePlatforms(){platforms=[];platformGroups=[];platformGrid=new Map();platformGroupById=new Map();generatedPlatformCells=new Set();platformGenerationX=Infinity;platformGenerationZ=Infinity;ensurePlatforms(0,0,true);canvas.dataset.platforms=String(platformGroups.length);}
  function platformGroupSupporting(entity){const support=platformSupportAt(entity.x,entity.z,(Number(entity.y)||0)+PLATFORM_STEP_HEIGHT+.08);return support.surface?platformGroupById.get(support.surface.groupId)||null:null;}
  function platformPursuitVector(entity,target){
    const entityY=Number(entity.y)||0,targetY=Number(target.y)||0,difference=targetY-entityY;if(Math.abs(difference)<.18){entity.platformRouteId='';return null;}const group=difference>0?platformGroupSupporting(target):platformGroupSupporting(entity);if(!group){entity.platformRouteId='';return null;}let waypoint;if(difference>0){const baseDistance=Math.hypot(entity.x-group.stairBase.x,entity.z-group.stairBase.z),climbing=entity.platformRouteId===group.id||entityY>.15||baseDistance<=1.1;if(climbing)entity.platformRouteId=group.id;waypoint=climbing?group.stairTop:group.stairBase;}else{const routeId=`down:${group.id}`,topDistance=Math.hypot(entity.x-group.stairTop.x,entity.z-group.stairTop.z),descending=entity.platformRouteId===routeId||entityY<group.height-.15||topDistance<=1.1;if(descending)entity.platformRouteId=routeId;waypoint=descending?group.stairBase:group.stairTop;}const dx=waypoint.x-entity.x,dz=waypoint.z-entity.z,distance=Math.hypot(dx,dz)||1;return{x:dx/distance,z:dz/distance};
  }
  ...
  function moveWithObstacles(entity,dx,dz,radius=.5){
    let clear=true;const steps=Math.max(1,Math.ceil(Math.hypot(dx,dz)/.32)),stepX=dx/steps,stepZ=dz/steps,playerControlled=entity===player;
    for(let step=0;step<steps;step++){let old=entity.x;entity.x+=stepX;if(pointBlockedByObstacle(entity.x,entity.z,radius)||platformVolumeBlocks(entity,entity.x,entity.z,radius)){entity.x=old;clear=false;}else{const support=platformSupportAt(entity.x,entity.z,(Number(entity.y)||0)+PLATFORM_STEP_HEIGHT+.05).height;if(support>(Number(entity.y)||0)&&(Number(entity.jumpVelocity)||0)<=0)entity.y=support;else if(!playerControlled&&(Number(entity.jumpVelocity)||0)===0)entity.y=support;}old=entity.z;entity.z+=stepZ;if(pointBlockedByObstacle(entity.x,entity.z,radius)||platformVolumeBlocks(entity,entity.x,entity.z,radius)){entity.z=old;clear=false;}else{const support=platformSupportAt(entity.x,entity.z,(Number(entity.y)||0)+PLATFORM_STEP_HEIGHT+.05).height;if(support>(Number(entity.y)||0)&&(Number(entity.jumpVelocity)||0)<=0)entity.y=support;else if(!playerControlled&&(Number(entity.jumpVelocity)||0)===0)entity.y=support;}}
    return clear;
  }
  ...
  function openFloorPosition(x,z,radius=.4,maxDistance=12){
    const blocked=(px,pz)=>pointBlockedByObstacle(px,pz,radius)||platformVolumeBlocks({y:0,size:Math.max(.55,radius*2)},px,pz,radius);if(!blocked(x,z))return{x,z};const angleOffset=(seedHash(`${state.seedCode}:open-floor:${x.toFixed(2)}:${z.toFixed(2)}`)%6283)/1000,step=Math.max(.55,radius*.55);for(let distance=step;distance<=maxDistance;distance+=step){const samples=Math.max(12,Math.ceil(TAU*distance/step));for(let i=0;i<samples;i++){const angle=angleOffset+i/samples*TAU,nx=x+Math.cos(angle)*distance,nz=z+Math.sin(angle)*distance;if(!blocked(nx,nz))return{x:nx,z:nz};}}const fallback={x,z};resolveObstacleOverlaps(fallback,radius);return fallback;
  }
  function placeOnOpenFloor(entity,x,z,radius=.4,maxDistance=12){const position=openFloorPosition(x,z,radius,maxDistance);entity.x=position.x;entity.z=position.z;if('y'in entity)entity.y=platformSupportAt(entity.x,entity.z,PLATFORM_STEP_HEIGHT).height;return entity;}
  ...
  const upgrades = [
    ...
    {id:'seismic',name:'Сейсмическое ядро',icon:'◎',type:'ОРУЖИЕ',rarity:'epic',max:8,when:()=>state.stats.level>=6,level:()=>state.weapons.seismic,desc:()=>state.weapons.seismic?'Увеличивает урон, радиус и скорость волны.':'Выпускает вокруг героя отталкивающую ударную волну.',apply:()=>state.weapons.seismic++},
    ...
    {id:'mirrordisc',name:'Зеркальный диск',icon:'◐',type:'ОРУЖИЕ',rarity:'epic',max:8,when:()=>state.stats.level>=7,level:()=>state.weapons.mirrordisc,desc:()=>state.weapons.mirrordisc?'Усиливает контактный урон и ускоряет отражение.':'Ранит ближайших врагов и отражает часть вражеских снарядов.',apply:()=>state.weapons.mirrordisc++},
    ...
    {id:'katana',name:'Катана',icon:'刃',type:'ОРУЖИЕ',rarity:'epic',max:8,when:()=>state.stats.level>=6,level:()=>state.weapons.katana,desc:()=>state.weapons.katana?'Увеличивает урон, ширину, дальность и темп удара.':'Рассекает дугой всех врагов перед героем.',apply:()=>state.weapons.katana++},
    ...
    {id:'multishot',name:'Двойная ставка',icon:'Ψ',type:'ХАРАКТЕРИСТИКА',rarity:'epic',max:5,desc:()=>'+1 снаряд поддерживаемому оружию и +1 Орбитальный клинок.',apply:()=>state.stats.projectiles++},
    ...
    {id:'velocity',name:'Гиперствол',icon:'↠',type:'ХАРАКТЕРИСТИКА',rarity:'common',max:8,desc:()=>'+18% скорости и +7% урона летящих снарядов.',apply:()=>{state.stats.projSpeed*=1.18;state.stats.projectileDamage*=1.07}},
    ...
    {id:'burnPower',name:'Белый фосфор',icon:'△',type:'ХАРАКТЕРИСТИКА',rarity:'rare',max:6,desc:()=>'+20% урона Огненного следа, Огнемёта и огня от трупов.',apply:()=>state.stats.burnPower++},
    ...
    {id:'echo',name:'Двигатель эха',icon:'∞',type:'ЛОМАЕТ ИГРУ',rarity:'legendary',max:5,desc:()=>'+18% шанс повторить атаку оружия.',apply:()=>state.stats.echo+=.18},
    {id:'criticalMass',name:'Критическая масса',icon:'✹',type:'ЛОМАЕТ ИГРУ',rarity:'legendary',max:4,desc:()=> 'Криты вызывают взрыв: 7% + 3% за ранг от нанесённого урона.',apply:()=>state.stats.critExplosion++},
    {id:'feedback',name:'Петля обратной связи',icon:'↻',type:'ЛОМАЕТ ИГРУ',rarity:'legendary',max:5,desc:()=> 'Убийства сокращают текущие перезарядки. Эффект ограничен за секунду.',apply:()=>state.stats.feedback++},
    {id:'blood',name:'Кровавая батарея',icon:'♦',type:'ЛОМАЕТ ИГРУ',rarity:'legendary',max:4,desc:()=> 'Каждые 200 убийств: +2% базового урона и 4 HP лечения за ранг. Бонус урона — до +200%.',apply:()=>state.stats.blood++},
    {id:'levelPower',name:'Корона экспоненты',icon:'♛',type:'ЛОМАЕТ ИГРУ',rarity:'legendary',max:3,desc:()=> 'Каждые 5 новых уровней дают +2% базового урона за ранг.',apply:()=>state.stats.levelPower++},
    ...
    {id:'hunterAnchor',name:'Якорь охотника',icon:'⚓',type:'ХАРАКТЕРИСТИКА',rarity:'rare',max:4,desc:()=> 'После 2 секунд неподвижности: +8% урона и +6% размера атак за ранг.',apply:()=>state.stats.hunterAnchor++},
    {id:'thermoSiphon',name:'Термосифон',icon:'♨+',type:'РЕЛИКВИЯ',rarity:'epic',max:4,desc:()=> '+5% урона от других атак по горящим врагам и 0,8% максимального HP за их убийство за ранг. Лечение — раз в секунду.',apply:()=>state.stats.thermoSiphon++},
    ...
    {id:'pocketForge',name:'Карманная кузница',icon:'⚒',type:'РЕЛИКВИЯ',rarity:'epic',max:3,desc:()=> 'На каждом 5-м уровне временно повышает случайное оружие на 1 уровень за ранг.',apply:()=>state.stats.pocketForge++},
    ...
    {id:'riftStabilizer',name:'Стабилизатор Разлома',icon:'⌛+',type:'ХАРАКТЕРИСТИКА',rarity:'rare',max:5,desc:()=>'+8% длительности зон; мгновенные зоны оставляют поле с 30% урона. Перезарядка зон +5% за ранг.',apply:()=>state.stats.riftStabilizer++},
    {id:'gildedLure',name:'Позолоченная приманка',icon:'♧',type:'РИСК',rarity:'legendary',max:1,desc:()=>'Шанс расходников повышается до максимальных 2%; элиты получают +8% здоровья.',apply:()=>{state.stats.dropLuck+=1/CONSUMABLE_BASE_CHANCE/100;state.stats.gildedLure=1}},
    ...
    {id:'unstableDuplicator',name:'Нестабильный дубликатор',icon:'Ⅱ?',type:'ЛОМАЕТ ИГРУ',rarity:'legendary',max:5,desc:()=>'+4% шанс за ранг повторить попадание с 60% урона.',apply:()=>state.stats.unstableDuplicator++},
  ...
  const bossRelics = [
    ...
    {id:'bossBlackMirror',name:'Чёрное зеркало',icon:'◇',type:'РЕЛИКВИЯ БОССА',rarity:'boss',max:1,desc:()=>'+35% шанс повторить атаку оружия.',apply:()=>state.stats.echo+=.35},
    {id:'bossWarArchive',name:'Архив войны',icon:'Ψ',type:'РЕЛИКВИЯ БОССА',rarity:'boss',max:1,desc:()=>'+2 снаряда, +3 пробития и +20% скорости снарядов.',apply:()=>{state.stats.projectiles+=2;state.stats.pierce+=3;state.stats.projSpeed*=1.2}},
  ...
  const evolutionUpgrades = [
    ...
    {id:'evoKatana',weapon:'katana',requires:'crit',name:'МУРАМАСА РАЗЛОМА',icon:'刃✦',type:'ЭВОЛЮЦИЯ',rarity:'evolved',max:1,desc:()=> 'Катана создаёт двойной спектральный разрез, увеличивая урон, дальность и ширину взмаха.'},
    {id:'evoFlamethrower',weapon:'flamethrower',requires:'burnPower',name:'ГЕЕННА',icon:'♨△',type:'ЭВОЛЮЦИЯ',rarity:'evolved',max:1,desc:()=> 'Огнемёт выпускает расширенную бело-золотую струю: прямой урон, дальность, ширина и горение значительно усиливаются.'}
  ...
  const CODEX_EVOLUTION_STATS = {
    ...
    evoGravity:['Урон 0','Длительность ×1,35','Радиус ×1,45','Притяжение ×1,80'],
    ...
    evoKatana:['Двойной спектральный разрез','Урон ×1,60','Дальность ×1,18','Ширина дуги ×1,25'],
    evoFlamethrower:['Прямой урон ×1,35','Дальность ×1,18','Ширина струи ×1,20','Урон горения ×1,60','Бело-золотое пламя']
  };
  ...
  const CODEX_ITEM_NOTES = {
    ...
    multishot:'Обычные снаряды и клинки получают полноценную копию. Дополнительные метеоры, заряды и лучи наносят 50% урона, молнии — 55%; мины и зоны не копируются.',
    pierce:'Увеличивает число целей, проходимых снарядом; урон не меняется.',
    velocity:'Усиливает летящие снаряды, но не влияет на зоны, ауры и контактные атаки.',
    duration:'Влияет на зоны, расходники и особые атаки, включая Огненный след.',
    ...
    echo:'Действует на всё оружие.',
    burnPower:'Усиливает Огненный след, огонь от трупов, струю и горение Огнемёта.',
    riftStabilizer:'Мгновенные зоны оставляют поле, которое один раз поражает вошедших врагов. Крио-импульс также дольше замедляет.',
    gildedLure:'Глобальный итоговый шанс расходников всё равно ограничен пределом 2%.',
    bossBlackMirror:'Действует на всё оружие.',
  ...
  const ITEM_UNLOCK_RULES = {
    ...
    burnPower:{weapons:['firetrail','flamethrower'],items:['corpseFire']},
    thermoSiphon:{weapons:['firetrail','flamethrower'],items:['corpseFire']},
    ...
    blackPowder:{weapons:['aura','meteor','frost','firetrail','flamethrower','mines','mortar','seismic','resonance'],items:['corpseFire','killNova','criticalMass'],evolutions:['evoLightning','evoRiftlance']},
  ...
  function startGame() {
    ...
    resetFloatingStick();
    audio.init();
    state=makeRun();player={x:0,z:0,y:0};jumpQueued=false;enemies=[];activeStandards=[];projectiles=[];enemyProjectiles=[];gems=[];particles=[];beams=[];zones=[];consumables=[];worldTotems=[];heroUnits=[];clearCombatTexts();generateMapObstacles();generatePlatforms();
  ...
  function returnMenu() {
    if(isCoop())closeCoop();
    resetFloatingStick();
    state={mode:'menu'};setCameraMode('overhead',false);jumpQueued=false;enemies=[];activeStandards=[];projectiles=[];enemyProjectiles=[];gems=[];particles=[];beams=[];zones=[];consumables=[];worldTotems=[];heroUnits=[];clearCombatTexts();obstacles=[];obstacleGrid=new Map();generatedObstacleCells=new Set();obstacleGenerationX=Infinity;obstacleGenerationZ=Infinity;platforms=[];platformGroups=[];platformGrid=new Map();platformGroupById=new Map();generatedPlatformCells=new Set();platformGenerationX=Infinity;platformGenerationZ=Infinity;mazeFlowField=new Map();mazeFlowKey='';canvas.dataset.platforms='0';canvas.dataset.mazePathCells='0';
  ...
  function pauseToggle() {
    ...
    if(state.mode==='playing'){resetFloatingStick();jumpQueued=false;state.mode='paused';releaseCameraPointerLock();$('#pauseScreen').classList.remove('hidden');}
  ...
  function updateUI(force=false) {
    ...
    const adaptiveHealth=Math.round(((adaptive.health||1)-1)*100),adaptiveDamage=Math.round(((adaptive.damage||1)-1)*100),adaptiveSpeed=Math.round(((adaptive.speed||1)-1)*100),analysisSeconds=Math.min(12,Math.floor(adaptive.dominanceTime||0)),adaptiveLabel=adaptiveHealth>1?`ДАВЛЕНИЕ +${adaptiveHealth}% HP · +${Math.max(0,adaptiveDamage)}% УР · +${Math.max(0,adaptiveSpeed)}% СКР`:adaptive.dominanceCandidate&&!adaptive.dominant?`АНАЛИЗ ДОМИНИРОВАНИЯ ${analysisSeconds}/12С`:adaptiveHealth<-1?`ПОДДЕРЖКА ${adaptiveHealth}% HP`:'НЕЙТРАЛЬНО';setHudText('#adaptiveDifficulty',adaptiveLabel);setHudText('#expectedLevel',`ОЖИД. УР. ${Math.round(adaptive.expectedLevel||1)}`);
  ...
  function post15Scales(time=state.time) {
    ...
    const strength=(state.difficulty?.rift??1)*.85,from=t<900?{health:1,damage:1,spawn:1,speed:1}:t<1200?{health:1.65,damage:1.25,spawn:1.35,speed:1.08}:{health:2.5,damage:1.6,spawn:1.75,speed:1.15},to=t<900?{health:1.65,damage:1.25,spawn:1.35,speed:1.08}:t<1200?{health:2.5,damage:1.6,spawn:1.75,speed:1.15}:{health:3.4,damage:2.15,spawn:1.98,speed:1.27},ramp=t<900?(t-600)/300:t<1200?(t-900)/300:clamp((t-1200)/600,0,1),scaled={};
  ...
  function adaptiveDifficultyTarget(time=state.time,dominantConfirmed=Boolean(state.adaptive?.dominant)){
    const adaptive=state.adaptive||{},expected=expectedPlayerLevel(time),actualCheckpoint=Math.floor((state.stats?.level||1)/5)*5,expectedCheckpoint=Math.floor(expected/5)*5,levelSpan=Math.max(15,expected*.18),levelAhead=clamp((actualCheckpoint-expectedCheckpoint)/levelSpan,0,1),levelBehind=clamp((expectedCheckpoint-actualCheckpoint)/levelSpan,0,1),build=adaptiveBuildPressure(time),baseSpawn=enemyGrowth(time).spawn*(state.difficulty?.spawn??1)*post15Scales(time).spawn,killRatio=(adaptive.killRate||0)/Math.max(1.5,baseSpawn),performance=clamp((killRatio-1.05)/.70,0,1),living=enemies.filter(enemy=>!enemy.dead),nearRatio=living.filter(enemy=>dist2(player,enemy)<=8**2).length/Math.max(1,living.length),crowdLimit=Math.max(24,Math.ceil(baseSpawn*8)),crowdControlled=living.length<=crowdLimit,dominanceCandidate=killRatio>=1.10&&crowdControlled&&nearRatio<=.08,stationary=runTimelineTime(time)>=360&&dominantConfirmed?clamp(((player.stationaryTime||0)-2)/6,0,1):0,effectiveLevelBehind=dominantConfirmed?0:levelBehind,healthRelief=levelBehind*(dominantConfirmed?.10:.08),adaptiveLevel=dominantConfirmed?levelAhead:0,adaptiveBuild=dominantConfirmed?build:0,adaptivePerformance=dominantConfirmed?performance:0,health=clamp(1+adaptiveLevel*.28+adaptiveBuild*.38+adaptivePerformance*.18+stationary*.20-healthRelief,.94,ADAPTIVE_HEALTH_MAX),damage=clamp(1+adaptiveLevel*.07+adaptiveBuild*.04+adaptivePerformance*.03+stationary*.04-effectiveLevelBehind*.02,.98,1.16),speed=clamp(1+adaptiveBuild*.035+adaptivePerformance*.04+stationary*.08,1,1.14),eliteBonus=adaptiveLevel*.05+adaptiveBuild*.08+adaptivePerformance*.06+stationary*.05,threatWeight=Math.round((adaptiveLevel+adaptiveBuild+adaptivePerformance+stationary)*3),pressure=clamp(adaptiveLevel*.34+adaptiveBuild*.32+adaptivePerformance*.20+stationary*.14,0,1);
    return{expected,levelAhead,levelBehind,effectiveLevelBehind,build,performance,stationary,killRatio,nearRatio,crowdLimit,crowdControlled,dominanceCandidate,dominant:dominantConfirmed,health,damage,speed,eliteBonus,threatWeight,pressure};
  }
  function updateAdaptiveScaling(dt){
    ...
    let target=adaptiveDifficultyTarget(state.time,adaptive.dominant);const wasDominant=Boolean(adaptive.dominant);if(target.dominanceCandidate){adaptive.dominanceTime=Math.min(12,(adaptive.dominanceTime||0)+dt);adaptive.dominanceLoss=0;if(adaptive.dominanceTime>=12)adaptive.dominant=true;}else{adaptive.dominanceTime=0;if(adaptive.dominant){adaptive.dominanceLoss=(adaptive.dominanceLoss||0)+dt;if(adaptive.dominanceLoss>=3){adaptive.dominant=false;adaptive.dominanceLoss=0;}}else adaptive.dominanceLoss=0;}if(Boolean(adaptive.dominant)!==wasDominant)target=adaptiveDifficultyTarget(state.time,adaptive.dominant);
    const response=1-Math.exp(-dt/10),nextHealth=lerp(adaptive.health||1,target.health,response),nextDamage=lerp(adaptive.damage||1,target.damage,response),nextSpeed=lerp(adaptive.speed||1,target.speed,response);adaptive.health=clamp(target.dominant?Math.max(1,nextHealth):nextHealth,.94,ADAPTIVE_HEALTH_MAX);adaptive.damage=target.dominant?Math.max(1,nextDamage):nextDamage;adaptive.speed=target.dominant?Math.max(1,nextSpeed):nextSpeed;adaptive.eliteBonus=lerp(adaptive.eliteBonus||0,target.eliteBonus,response);adaptive.threatWeight=lerp(adaptive.threatWeight||0,target.threatWeight,response);adaptive.targetHealth=target.health;adaptive.targetDamage=target.damage;adaptive.targetSpeed=target.speed;adaptive.expectedLevel=target.expected;adaptive.levelPressure=target.levelAhead;adaptive.buildPressure=target.build;adaptive.performancePressure=target.performance;adaptive.stationaryPressure=target.stationary;adaptive.pressure=target.pressure;adaptive.dominanceCandidate=target.dominanceCandidate;adaptive.crowdControlled=target.crowdControlled;adaptive.crowdLimit=target.crowdLimit;
  }
  ...
  function fireBossVolley(boss) {
    ...
    for(let i=0;i<count;i++){
      ...
      spawnEnemyProjectile({x:boss.x,z:boss.z,y:(Number(boss.y)||0)+1.1+boss.size*.35,vx:Math.cos(a)*speed,vz:Math.sin(a)*speed,life:5.5,damage:boss.damage*.42,bossDamage:true,size:.34,color:i===(count-1)/2?COLORS.pink:COLORS.red,spin:Math.random()*TAU});
    }
  ...
  function fireBossRadial(boss,count=12,damage=.34) {
    ...
    for(let i=0;i<count;i++){const a=i/count*TAU+boss.seed;spawnEnemyProjectile({x:boss.x,z:boss.z,y:(Number(boss.y)||0)+1+boss.size*.3,vx:Math.cos(a)*speed,vz:Math.sin(a)*speed,life:5,damage:boss.damage*damage,bossDamage:true,size:.29,color:COLORS.red,spin:nativeRandom()*TAU});}
  ...
  function fireEnemyShot(e) {
    ...
    for(let i=0;i<count;i++){
      ...
      spawnEnemyProjectile({x:e.x+Math.cos(a)*(e.size+.25),z:e.z+Math.sin(a)*(e.size+.25),y:(Number(e.y)||0)+.85+e.size*.4,vx:Math.cos(a)*speed,vz:Math.sin(a)*speed,life:5,damage:e.damage*.58,size:.25,color:COLORS.red,spin:Math.random()*TAU});
    }
  ...
  function spawnCombatText(bucket){
    if(!combatNumbersEnabled||!combatTextLayer||state.mode==='menu')return;const priority=bucket.priority||1,maxVisible=28;
  ...
  function queueCombatText(type,x,z,y,amount,key,critical=false,priority=1){
    if(!combatNumbersEnabled||!Number.isFinite(amount)||amount<.05||state.mode==='menu')return;const bucketKey=`${type}:${key}`,existing=combatTextBuckets.get(bucketKey);
  ...
  function killProcCoefficient(source){if(source==='flamethrower')return .35;if(NON_RECURSIVE_SOURCES.has(source))return 0;if(['firetrail','storm','voidPressure','mines','mortar','seismic'].includes(source))return .35;return 1;}
  ...
  function damageEnemy(e,amount,source='normal',canCrit=true,preScaled=false) {
    const bossLike=e.boss||e.miniboss;if(e.dead||e.burrowWindup>0||bossLike&&e.bossPhaseInvuln>0)return 0;const s=state.stats,fireSource=['firetrail','flamethrower','flamethrowerBurn','corpseFire','pyroblast'].includes(source);let crit=canCrit&&gameRandom()<s.crit+(state.buffs.duelist>0?.25:0);
  ...
  const AREA_DAMAGE_SOURCES=new Set(['aura','meteor','frost','firetrail','flamethrower','flamethrowerBurn','storm','voidPressure','corpseFire','killnova','critburst','riftScar','mines','mineLink','mortar','seismic','resonance','katana','pyroblast','guardianNova','mimicSpecial']);
  ...
  function spawnStabilizedField({owner=activeActorId,x,z,radius,baseLife,damage,source,color,initialHits=[],slowDuration=0,slowPower=0}){
    if(!(state.stats.riftStabilizer||0))return;const life=zoneDuration(baseLife);zones.push({owner,weapon:source,x,z,radius,life,max:life,color,kind:'pulse',stabilized:true,damage:damage*.3,source,hit:new Set(initialHits),slowDuration,slowPower});
  }
  ...
  function fireAura() {
    const l=weaponLevel('aura');if(!l)return;const evolved=hasEvolution('aura'),radius=areaRadius((3.1+l*.48)*(evolved?1.45:1)),copies=1+echoCopies(),damage=(9+l*6)*(evolved?1.6:1);
    for(let copy=0;copy<copies;copy++){
      const fieldRadius=radius*(1+copy*.025),color=copy?COLORS.cyan:COLORS.violet,initialHits=[];zones.push({owner:activeActorId,x:player.x,z:player.z,radius:fieldRadius,life:.35+copy*.04,max:.35+copy*.04,color,kind:'pulse'});
      for(const e of enemyCandidates(player.x,player.z,fieldRadius))if(!e.dead&&dist2(player,e)<fieldRadius*fieldRadius){initialHits.push(e.nid);damageEnemy(e,damage,'aura');}
      spawnStabilizedField({x:player.x,z:player.z,radius:fieldRadius,baseLife:.7,damage,source:'aura',color,initialHits});
    }
  ...
  function fireFrost() {
    const l=weaponLevel('frost');if(!l)return;const evolved=hasEvolution('frost'),radius=areaRadius((4+l*.48)*(evolved?1.4:1)),copies=1+echoCopies(),damage=(12+l*6)*(evolved?1.45:1),slowDuration=zoneDuration((2+l*.35)*(evolved?1.45:1)),slowPower=evolved?.82:clamp(.42+l*.035,.42,.72);
    for(let copy=0;copy<copies;copy++){
      const fieldRadius=radius*(1+copy*.03),color=copy?COLORS.violet:COLORS.cyan,initialHits=[];zones.push({owner:activeActorId,x:player.x,z:player.z,radius:fieldRadius,life:.5+copy*.04,max:.5+copy*.04,color,kind:'frost'});
      for(const e of enemyCandidates(player.x,player.z,fieldRadius))if(!e.dead&&dist2(player,e)<fieldRadius*fieldRadius){initialHits.push(e.nid);damageEnemy(e,damage,'frost');e.slow=Math.max(e.slow,slowDuration);e.slowPower=Math.max(e.slowPower||0,slowPower);}
      spawnStabilizedField({x:player.x,z:player.z,radius:fieldRadius,baseLife:1,damage,source:'frost',color,initialHits,slowDuration,slowPower});
    }
  ...
  function fireResonance() {
    const l=weaponLevel('resonance'),evolved=hasEvolution('resonance'),copies=1+echoCopies();state.stats.resonanceStacks=Math.min(10,(state.stats.resonanceStacks||0)+1);const stacks=state.stats.resonanceStacks,radius=areaRadius(4.4+l*.32),damage=(12+(l-1)*4)*(1+stacks*.15);for(let copy=0;copy<copies;copy++){const fieldRadius=radius*(1+copy*.025),color=copy?COLORS.cyan:COLORS.violet,initialHits=[];zones.push({owner:activeActorId,weapon:'resonance',x:player.x,z:player.z,radius:fieldRadius,life:.46,max:.46,color,kind:'pulse'});for(const e of enemyCandidates(player.x,player.z,fieldRadius))if(!e.dead&&dist2(player,e)<fieldRadius*fieldRadius){initialHits.push(e.nid);damageEnemy(e,damage,'resonance');}spawnStabilizedField({x:player.x,z:player.z,radius:fieldRadius,baseLife:.8,damage,source:'resonance',color,initialHits});}if(evolved&&stacks===10){const finalRadius=areaRadius(radius*1.5),finalDamage=damage*1.8,initialHits=[];zones.push({owner:activeActorId,weapon:'resonance',x:player.x,z:player.z,radius:finalRadius,life:.7,max:.7,color:COLORS.white,kind:'pulse'});for(const e of enemyCandidates(player.x,player.z,finalRadius))if(!e.dead&&dist2(player,e)<finalRadius*finalRadius){initialHits.push(e.nid);damageEnemy(e,finalDamage,'resonance');}spawnStabilizedField({x:player.x,z:player.z,radius:finalRadius,baseLife:1,damage:finalDamage,source:'resonance',color:COLORS.white,initialHits});state.stats.resonanceStacks=0;}audio.tone(190+stacks*30,.14,'sine',.018);
  }
  function fireKatana() {
    const l=weaponLevel('katana');if(!l)return;const evolved=hasEvolution('katana'),range=areaRadius((4.4+l*.35)*(evolved?1.18:1)),aim=Number.isFinite(player.dir)?player.dir:Number.isFinite(player.moveDir)?player.moveDir:0,halfArc=(.72+l*.025)*(evolved?1.25:1),damage=(26+(l-1)*7)*(evolved?1.6:1),copies=1+echoCopies();
    for(let copy=0;copy<copies;copy++){const offset=copy?Math.ceil(copy/2)*(copy%2?.045:-.045):0,dir=aim+offset,forwardX=Math.cos(dir),forwardZ=Math.sin(dir);for(const enemy of enemyCandidates(player.x,player.z,range+4)){if(enemy.dead)continue;const dx=enemy.x-player.x,dz=enemy.z-player.z,distance=Math.hypot(dx,dz);if(distance>range+enemy.size||lineBlockedByObstacle(player.x,player.z,enemy.x,enemy.z,.12))continue;const angularPadding=Math.asin(clamp(enemy.size/Math.max(enemy.size+.001,distance),0,.8)),angle=Math.acos(clamp((dx*forwardX+dz*forwardZ)/Math.max(.001,distance),-1,1));if(angle<=halfArc+angularPadding)damageEnemy(enemy,damage,'katana');}const life=.38+copy*.025;zones.push({owner:activeActorId,weapon:'katana',x:player.x,z:player.z,radius:range,life,max:life,color:copy?COLORS.violet:evolved?COLORS.white:COLORS.cyan,kind:'katana',dir,evolved,seed:gameRandom()*TAU});}
    audio.tone(evolved?780:640,.085,'triangle',.025);audio.tone(evolved?1120:920,.045,'sine',.016);
  }
  function igniteEnemy(enemy,damage,duration,owner=activeActorId){
    if(enemy.dead)return;enemy.burnTime=Math.max(enemy.burnTime||0,duration);enemy.flamethrowerBurnTime=Math.max(enemy.flamethrowerBurnTime||0,duration);if(damage>=(enemy.flamethrowerBurnDamage||0)){enemy.flamethrowerBurnDamage=damage;enemy.flamethrowerBurnOwner=owner;}if(!Number.isFinite(enemy.flamethrowerBurnTick))enemy.flamethrowerBurnTick=.36;
  }
  function fireFlamethrower() {
    const l=weaponLevel('flamethrower');if(!l)return;const evolved=hasEvolution('flamethrower'),baseRange=areaRadius((5.8+l*.32)*(evolved?1.18:1)),halfAngle=(.30+l*.018)*(evolved?1.20:1),aim=Number.isFinite(player.dir)?player.dir:Number.isFinite(player.moveDir)?player.moveDir:0,fireScale=1+(state.stats.burnPower||0)*.20,directDamage=(3+(l-1)*1.05)*(evolved?1.35:1)*fireScale,burnDamage=(2+(l-1)*.65)*(evolved?1.60:1)*fireScale,burnDuration=1.25+l*.07,copies=1+echoCopies();
    for(let copy=0;copy<copies;copy++){const offset=copy?Math.ceil(copy/2)*(copy%2?.055:-.055):0,dir=aim+offset,forwardX=Math.cos(dir),forwardZ=Math.sin(dir),visualRange=obstacleRayDistance(player.x,player.z,forwardX,forwardZ,baseRange,.16),visualWidth=Math.max(1.05,visualRange*Math.tan(halfAngle)*2);for(const enemy of enemyCandidates(player.x,player.z,baseRange+4)){if(enemy.dead)continue;const dx=enemy.x-player.x,dz=enemy.z-player.z,distance=Math.hypot(dx,dz),forward=dx*forwardX+dz*forwardZ,lateral=Math.abs(dx*forwardZ-dz*forwardX),coneWidth=.24+Math.max(0,forward)*Math.tan(halfAngle);if(distance>baseRange+enemy.size||forward< -enemy.size*.35||lateral>coneWidth+enemy.size||lineBlockedByObstacle(player.x,player.z,enemy.x,enemy.z,.12))continue;enemy.burnTime=Math.max(enemy.burnTime||0,burnDuration);damageEnemy(enemy,directDamage,'flamethrower',false);if(!enemy.dead)igniteEnemy(enemy,burnDamage,burnDuration,activeActorId);}const life=.18+copy*.008;zones.push({owner:activeActorId,weapon:'flamethrower',x:player.x+forwardX*visualRange*.5,z:player.z+forwardZ*visualRange*.5,radius:visualRange,length:visualRange,width:visualWidth,life,max:life,color:evolved?COLORS.white:COLORS.amber,kind:'flamethrower',dir,evolved,seed:gameRandom()*TAU});}
    if((player.flamethrowerSoundClock||0)<=state.time){player.flamethrowerSoundClock=state.time+.27;audio.tone(evolved?185:145,.12,'sawtooth',.018);audio.tone(evolved?740:520,.045,'triangle',.009);}
  }
  ...
  function updateWeapons(dt) {
    ...
    if(state.weapons.katana&&state.cooldowns.katana<=0){fireKatana();state.cooldowns.katana=(1.55-weaponLevel('katana')*.065)/rate;}
    if(state.weapons.flamethrower&&state.cooldowns.flamethrower<=0){fireFlamethrower();state.cooldowns.flamethrower=(.15-weaponLevel('flamethrower')*.005)/rate;}
    if(state.weapons.mirrordisc)updateMirrorDisc(dt);
    const l=weaponLevel('orbit');
    if(l){const evolved=hasEvolution('orbit'),count=2+Math.floor(l/2)+extraProjectileCount()+(evolved?3:0),r=(2.15+l*.12)*(evolved?1.15:1),now=state.time*(1.7+l*.09)*(evolved?1.18:1);
  ...
  function updateZones(dt) {
    for(let i=zones.length-1;i>=0;i--){const z=zones[i];if((z.delay||0)>0){z.delay-=dt;continue;}z.life-=dt;
      if(z.kind==='pulse'&&z.stabilized&&z.life>0){z.hit=z.hit instanceof Set?z.hit:new Set();withActor(actorById(z.owner),()=>{for(const e of enemyCandidates(z.x,z.z,z.radius)){if(e.dead||z.hit.has(e.nid)||dist2(z,e)>=z.radius*z.radius)continue;z.hit.add(e.nid);damageEnemy(e,z.damage,z.source||z.weapon||'normal');if(!e.dead&&z.slowDuration>0){e.slow=Math.max(e.slow,z.slowDuration);e.slowPower=Math.max(e.slowPower||0,z.slowPower||0);}}});}
      if(z.kind==='gravity'){
      ...
      if(z.kind==='meteor'&&z.life<=0&&!z.exploded){z.exploded=true;withActor(actorById(z.owner),()=>{const initialHits=[];for(const e of enemyCandidates(z.x,z.z,z.radius))if(!e.dead&&dist2(z,e)<z.radius*z.radius){initialHits.push(e.nid);damageEnemy(e,z.damage,z.weapon||'meteor');}spawnStabilizedField({owner:z.owner,x:z.x,z:z.z,radius:z.radius,baseLife:1.2,damage:z.damage,source:z.weapon||'meteor',color:z.color,initialHits});});burst(z.x,z.z,z.color,18,1.4);shake=Math.min(1.2,shake+.35);audio.tone(85,.2,'sawtooth',.04);z.life=.28;z.max=.28;z.kind='blast';}
      if(z.kind==='mortar'&&z.life<=0&&!z.exploded){z.exploded=true;withActor(actorById(z.owner),()=>{const initialHits=[];for(const e of enemyCandidates(z.x,z.z,z.radius))if(!e.dead&&dist2(z,e)<z.radius*z.radius){initialHits.push(e.nid);damageEnemy(e,z.damage,'mortar');}spawnStabilizedField({owner:z.owner,x:z.x,z:z.z,radius:z.radius,baseLife:1,damage:z.damage,source:'mortar',color:z.color,initialHits});});burst(z.x,z.z,z.color,16,1.2);z.life=.28;z.max=.28;z.kind='blast';}
  ...
  function updateEnemies(dt) {
    ...
    for(const e of enemies){if(e.dead)continue;e.hit=Math.max(0,e.hit-dt);e.orbitHit=Math.max(0,e.orbitHit-dt);e.mirrorHit=Math.max(0,(e.mirrorHit||0)-dt);e.slow=Math.max(0,e.slow-dt);e.burnTime=Math.max(0,(e.burnTime||0)-dt);e.flamethrowerBurnTime=Math.max(0,(e.flamethrowerBurnTime||0)-dt);if(e.flamethrowerBurnTime>0){e.burnTime=Math.max(e.burnTime,Math.min(.65,e.flamethrowerBurnTime));e.flamethrowerBurnTick=(e.flamethrowerBurnTick??.36)-dt;if(e.flamethrowerBurnTick<=0){const owner=actorById(e.flamethrowerBurnOwner);if(owner)withActor(owner,()=>damageEnemy(e,e.flamethrowerBurnDamage||0,'flamethrowerBurn',false));e.flamethrowerBurnTick+=.36;}}else{e.flamethrowerBurnDamage=0;e.flamethrowerBurnOwner='';e.flamethrowerBurnTick=.36;}if(e.dead)continue;if(e.burnTime<=0)e.pyroBurnStacks=0;e.bossPhaseInvuln=Math.max(0,(e.bossPhaseInvuln||0)-dt);
      ...
      if(bossLike&&e.bossWaveWindup>0){/* Босс замирает на коротком читаемом замахе. */}
        ...
        const move=platformRoute?1:clearPath?(d>14?1:d<8?-.72:0):1,strafe=platformRoute?0:clearPath?Math.sin(state.time*1.7+e.seed)*.2:0;moveWithObstacles(e,(moveX*move-moveZ*strafe)*speed*dt,(moveZ*move+moveX*strafe)*speed*dt,radius);
      ...
      if(d<meleeRange&&Math.abs((Number(e.y)||0)-(Number(targetPlayer.y)||0))<Math.max(1.05,e.size*.9)){withActor(target,()=>{const canVampire=e.affix==='vampiric'&&state.invuln<=0&&state.buffs.immortal<=0;hurtPlayer(e.damage*contactMult,e);if(canVampire)e.hp=Math.min(e.maxHp,e.hp+e.maxHp*.12)});}
    }
    resolveEnemyCrowding(dt);for(const enemy of enemies)if(!enemy.dead){if(obstacles.length)resolveObstacleOverlaps(enemy,Math.max(.3,enemy.size*.52));const support=platformSupportAt(enemy.x,enemy.z,(Number(enemy.y)||0)+PLATFORM_STEP_HEIGHT+.05).height;enemy.y=support;}
    ...
    const bossTimes=scheduledBossTimes();if(state.bossIndex<bossTimes.length&&timeline>=bossTimes[state.bossIndex]){spawnBoss('',state.bossIndex===0?.6:1);state.bossIndex++;}
  }
  ...
  function updateThreat() {
    ...
    const oldTier=state.threatTier,oldGrowth=enemyGrowth(now,oldTier),newGrowth=enemyGrowth(now,nextTier),damageScale=newGrowth.damage/oldGrowth.damage,speedScale=(1+nextTier*.008*(state.difficulty.scaling??1))/(1+oldTier*.008*(state.difficulty.scaling??1));
    state.threatTier=nextTier;
    for(const e of enemies){if(e.dead)continue;e.damage*=damageScale;e.speed*=speedScale;}
  ...
  function updateLateScale() {
    ...
    const target=post15Scales(),damageRatio=target.damage/state.lateDamageScale;
    if(Math.abs(damageRatio-1)>.000001)for(const e of enemies){if(e.dead)continue;e.damage*=damageRatio;}
    state.lateHealthScale=target.health;state.lateDamageScale=target.damage;
  ...
  function updateJumpMotion(entity,dt,jumpPressed,moving){
    entity.y=Math.max(0,Number(entity.y)||0);entity.jumpVelocity=Number(entity.jumpVelocity)||0;entity.bhopSpeed=clamp(Number(entity.bhopSpeed)||1,1,BHOP_MAX_SPEED);entity.bhopGrace=Math.max(0,Number(entity.bhopGrace)||0);entity.jumpBuffer=Math.max(0,(Number(entity.jumpBuffer)||0)-dt);if(jumpPressed)entity.jumpBuffer=JUMP_BUFFER;
    let support=platformSupportAt(entity.x,entity.z,entity.y+PLATFORM_STEP_HEIGHT+.06).height,grounded=entity.jumpVelocity===0&&Math.abs(entity.y-support)<=.06;
    if(grounded){entity.y=support;entity.jumpVelocity=0;entity.bhopGrace=Math.max(0,entity.bhopGrace-dt);}
    else{const previousY=entity.y;entity.jumpVelocity-=JUMP_GRAVITY*dt;const nextY=entity.y+entity.jumpVelocity*dt,landing=platformSupportAt(entity.x,entity.z,previousY+.06).height;if(entity.jumpVelocity<=0&&nextY<=landing&&previousY>=landing-.08){entity.y=landing;entity.jumpVelocity=0;entity.bhopGrace=BHOP_GRACE;grounded=true;support=landing;}else entity.y=Math.max(0,nextY);}
    if(!moving&&grounded){entity.bhopSpeed=1;entity.bhopGrace=0;}
    if(grounded&&entity.jumpBuffer>0){const chained=moving&&entity.bhopGrace>0;entity.bhopSpeed=chained?Math.min(BHOP_MAX_SPEED,entity.bhopSpeed+BHOP_GAIN):1;entity.jumpVelocity=JUMP_VELOCITY;entity.y=support+.001;entity.jumpBuffer=0;entity.bhopGrace=0;grounded=false;}
    else if(grounded&&entity.bhopGrace<=0)entity.bhopSpeed=1;
    return entity.bhopSpeed;
  }
  function updatePlayer(dt,input=null) {
    ...
    const l=Math.hypot(x,z),moving=l>0,contractSpeed=state.stats.bloodContract&&state.stats.hp<state.stats.maxHp*.5?1.12:1,bhopSpeed=updateJumpMotion(player,dt,!isCoop()&&Boolean(movement.jump),moving),moveBuff=speedBuffMultiplier()*contractSpeed*bhopSpeed;player.moving=moving;if(l>0){x/=Math.max(1,l);z/=Math.max(1,l);moveWithObstacles(player,x*state.stats.speed*moveBuff*dt,z*state.stats.speed*moveBuff*dt,.52);player.moveDir=Math.atan2(z,x);}ensureMapObstacles(player.x,player.z);if(!isCoop())ensurePlatforms(player.x,player.z);
  ...
  function render(time) {
    ...
    if(menu){
      ...
      const playerAttackAlpha=(playerAttacksFaded?ATTACK_OVERLOAD_ALPHA:1)*attackOpacity;
      ...
      let visiblePlatforms=0,hiddenPlatforms=0;const platformColors=[[theme.groundAlt[0]*1.32,theme.groundAlt[1]*1.32,theme.groundAlt[2]*1.32,1],[theme.ground[0]*1.6,theme.ground[1]*1.6,theme.ground[2]*1.6,1],[theme.line[0]*1.2,theme.line[1]*1.2,theme.line[2]*1.2,1]];for(const group of platformGroups){if((group.x-focus.x)**2+(group.z-focus.z)**2>54**2)continue;const hidden=!firstPerson&&(Number(player.y)||0)<group.height-.5&&Math.abs(player.x-group.x)<group.hx-.18&&Math.abs(player.z-group.z)<group.hz-.18;if(hidden){hiddenPlatforms++;continue;}visiblePlatforms++;for(const surface of group.surfaces){const color=platformColors[surface.variant%platformColors.length],width=surface.hx*2,depth=surface.hz*2;if(surface.kind==='deck'){add('cube',surface.x,surface.y-surface.thickness*.5,surface.z,width,surface.thickness,depth,0,color);add('cube',surface.x,surface.y+.035,surface.z,width*1.025,.07,depth*1.025,0,[...theme.accent,.82]);}else{add('cube',surface.x,surface.y*.5,surface.z,width,surface.y,depth,0,color);add('cube',surface.x,surface.y+.025,surface.z,width*1.02,.05,depth*1.02,0,[...theme.secondary,.72]);}}}canvas.dataset.visiblePlatforms=String(visiblePlatforms);canvas.dataset.hiddenPlatforms=String(hiddenPlatforms);
      ...
      for(const actor of renderActors)if(!hideActor(actor)){const ground=platformSupportAt(actor.entity.x,actor.entity.z,(Number(actor.entity.y)||0)+.08).height;add('cylinder',actor.entity.x,ground+.03,actor.entity.z,1.35,.04,1.35,0,COLORS.shadow);}
      for(const e of frameEnemies){const ground=platformSupportAt(e.x,e.z,(Number(e.y)||0)+.08).height;add('cylinder',e.x,ground+.025,e.z,e.size*1.5,.035,e.size*1.5,0,COLORS.shadow);}
      if(rtx){
        for(const actor of renderActors)if(!hideActor(actor)){const ground=platformSupportAt(actor.entity.x,actor.entity.z,(Number(actor.entity.y)||0)+.08).height;add('cylinder',actor.entity.x+.38,ground+.018,actor.entity.z+.28,1.8,.025,1.8,-.25,[0,0,.012,.2]);add('cylinder',actor.entity.x,ground+.012,actor.entity.z,7,.018,7,0,[.04,.58,.86,.055]);}
      ...
      for(const actor of renderActors){if(hideActor(actor))continue;const body=actor.entity,jumpY=Math.max(0,Number(body.y)||0),hc=HEROES[actor.hero]?.color||COLORS.cyan,blink=actor.invuln>0&&Math.floor(actor.invuln*20)%2===0;
        const usePocket=pocketReady&&actor.local;if(usePocket)pocketVisual={x:body.x,y:jumpY,z:body.z,dir:body.dir||0,moving:Boolean(body.moving),menu:false,visible:!blink};
        if(!blink&&!usePocket){
          add('cylinder',body.x,.69+jumpY,body.z,.98,1.28,.98,body.dir||0,[hc[0]*.52,hc[1]*.52,hc[2]*.52,1]);
          add('ring',body.x,1.12+jumpY,body.z,1.12,.14,1.12,-time*.0022,COLORS.brass);
          add('octa',body.x,1.48+jumpY,body.z,.61,.68,.61,time*.002,hc);
          add('octa',body.x,1.49+jumpY,body.z,.28,.31,.28,-time*.0032,COLORS.white);
          const dir=body.dir||0,sideX=Math.cos(dir+Math.PI/2),sideZ=Math.sin(dir+Math.PI/2);
          add('octa',body.x+sideX*.55,1.06+jumpY,body.z+sideZ*.55,.24,.34,.24,dir,COLORS.brass);
          add('octa',body.x-sideX*.55,1.06+jumpY,body.z-sideZ*.55,.24,.34,.24,dir,COLORS.brass);
          add('cube',body.x+Math.cos(dir)*.62,1.45+jumpY,body.z+Math.sin(dir)*.62,.46,.14,.18,-dir,COLORS.cyan);
        }
        if(actor.buffs.immortal>0||actor.stats.shield>0){const shieldColor=actor.buffs.immortal>0?[1,.75,.2,.32]:[.2,1,.84,.23];add('ring',body.x,.62+jumpY,body.z,1.95,1.22,1.95,time*.002,shieldColor);add('ring',body.x,.62+jumpY,body.z,1.95,1.22,1.95,-time*.0028,shieldColor);}
      }
      ...
      for(const e of frameEnemies){const visualTarget=closestActor(e)?.entity||player,c=e.hit?COLORS.white:e.color,y=(Number(e.y)||0)+e.size*.7+Math.sin(time*.004+e.seed)*.05,a=Math.atan2(visualTarget.z-e.z,visualTarget.x-e.x),shape=e.type==='runner'||e.type==='charger'||e.type==='burrower'?'pyramid':e.type==='shooter'||e.type==='absorber'?'cube':e.type==='splitter'||e.type==='phaser'?'octa':'cylinder',distanceSquared=dist2(e,focus),near=distanceSquared<18**2,medium=distanceSquared<32**2,important=e.boss||e.miniboss||e.elite||e.nid===localTargetNid||e.type==='standard'||e.type==='absorber',modelEnemy=monsterModelNids.has(e.nid);
        ...
        if(e.flamethrowerBurnTime>0&&(near||important)){const flamePulse=.82+Math.sin(time*.018+e.seed)*.18,flameColor=[1,.54,.06,.58];add('pyramid',e.x,y+e.size*.9,e.z,e.size*.42*flamePulse,e.size*1.25*flamePulse,e.size*.42*flamePulse,time*.004+e.seed,flameColor);if(near)add('octa',e.x+Math.sin(time*.006+e.seed)*e.size*.36,y+e.size*1.65,e.z+Math.cos(time*.005+e.seed)*e.size*.36,e.size*.13,e.size*.22,e.size*.13,-time*.006,[1,.88,.28,.72]);}
  ...
  function currentInput(){let x=(keys.KeyD||keys.ArrowRight?1:0)-(keys.KeyA||keys.ArrowLeft?1:0)+touchMove.x,z=(keys.KeyS||keys.ArrowDown?1:0)-(keys.KeyW||keys.ArrowUp?1:0)+touchMove.z,l=Math.hypot(x,z);if(l>1){x/=l;z/=l;}const jump=jumpQueued;jumpQueued=false;return{...cameraRelativeInput(x,z),jump};}
...
clear                        очистить консоль`;
  ...
  addEventListener('keydown',e=>{
    ...
    if(e.code==='Space'&&!e.repeat&&state.mode==='playing'&&!isCoop())jumpQueued=true;keys[e.code]=true;
  ...
  addEventListener('blur',()=>{keys={};jumpQueued=false;resetFloatingStick();if(state.mode==='playing'&&!isCoop())pauseToggle();});
  ...
  function selectCombatNumbers(mode,sound=true){combatNumbersEnabled=mode!=='off';$$('.combat-numbers-option').forEach(option=>{const selected=option.dataset.combatNumbers===(combatNumbersEnabled?'on':'off');option.classList.toggle('selected',selected);option.setAttribute('aria-pressed',String(selected));});if(combatTextLayer)combatTextLayer.dataset.enabled=String(combatNumbersEnabled);canvas.dataset.combatNumbers=combatNumbersEnabled?'on':'off';if(!combatNumbersEnabled)clearCombatTexts();try{localStorage.setItem('riftCombatNumbers',combatNumbersEnabled?'on':'off')}catch(_error){}if(sound){audio.init();audio.tone(combatNumbersEnabled?560:240,.09,'triangle',.022);}}
  $$('.combat-numbers-option').forEach(option=>option.addEventListener('click',()=>selectCombatNumbers(option.dataset.combatNumbers)));
  $('#renderScaleInput').addEventListener('input',event=>setRenderScale(Number(event.currentTarget.value)/100));
  $('#attackOpacityInput').addEventListener('input',event=>setAttackOpacity(Number(event.currentTarget.value)/100));
  ...
  canvas.dataset.telemetry=TELEMETRY_ENABLED?'on':'off';setRenderScale(renderScale,false);setAttackOpacity(attackOpacity,false);syncChallengeMenu();selectRunPace(selectedRunPace,false);selectMap(selectedMap,false);selectGraphics(selectedGraphics,false);selectCombatNumbers(combatNumbersEnabled?'on':'off',false);syncMenuSummary();showMenuPage('home','forward',false);loadBest();requestAnimationFrame(loop);
})();
